...
class CausalConv3d(nn.Conv3d):
    ...
    def forward(self, x, cache_x=None):
        padding = list(self._padding) # [1, 1, 1, 1, 2, 0] 按照w(left, right), h(top, bottom), t(front, back)的顺序排列
        if cache_x is not None and self._padding[4] > 0:
            cache_x = cache_x.to(x.device)
            x = torch.cat([cache_x, x], dim=2)
            padding[4] -= cache_x.shape[2]
        x = F.pad(x, padding)   # 首个chunk没有cache 左补2个0
...
class Encoder3d_38(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        if feat_cache is not None: # 应该是都走这里 因为在VideoVAE38_的encoder里面clear_cache初始化后feat_cache是[None]*layer
            idx = feat_idx[0]   # feat_idx是单元素list（如 [0]），作为"当前走到第几个CausalConv3d"的游标。
            cache_x = x[:, :, -CACHE_T:, :, :].clone()
            if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                cache_x = torch.cat(
                    [
            ...
            x = self.conv1(x, feat_cache[idx])
            feat_cache[idx] = cache_x # 更新第0层的feat_cache(这里还是image经过patch的), 供下一个chunk使用
            feat_idx[0] += 1
        else:
            x = self.conv1(x)

        ## downsamples
        for layer in self.downsamples:  # 每经过一个CausalConv3d层就更新一层feat_cache和feat_idx(层数+1)
            if feat_cache is not None:
                x, feat_cache, feat_idx = layer(x, feat_cache, feat_idx)
            else:
                x = layer(x)
...
class WanVideoVAE(nn.Module):
    ...
    def encode(self, videos, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        ...
        for video in videos:
            ...
            if tiled:   # tile_size换算到像素空间就是480,832; 所以如果输入是480,832并没有tile
...
class VideoVAE38_(VideoVAE_):
    ...
    def encode(self, x, scale): # 都是1 4 4 4 4...的分chunk进行encode, 不会整段encode
        self.clear_cache()  # 会把self._enc_feat_map(记录时间段之间的cache)变成[None]*层数
        x = patchify(x, patch_size=2)
        t = x.shape[2]
        iter_ = 1 + (t - 1) // 4
        for i in range(iter_): # 时间1+4+4...分chunk进行循环encode
            self._enc_conv_idx = [0]    # 记录cache的conv网络层数槽位, 每个chunk都从0开始
            if i == 0:
                out, self._enc_feat_map, self._enc_conv_idx = self.encoder(x[:, :, :1, :, :],
                                   feat_cache=self._enc_feat_map,
                                   feat_idx=self._enc_conv_idx)
